      ...
      <header className="bg-white dark:bg-slate-900 shadow-sm border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">{party.party_name}</h1>
          ...
          <form action={async () => {
            ...
            <button type="submit" className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="p-6 sm:p-8 flex flex-col items-center justify-center text-center">
            <h2 className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Current Ledger Balance</h2>
            <div className={`text-4xl sm:text-5xl font-bold ${currentBalance < 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
            ...
            {invoices && invoices.length > 0 ? (
              <ul className="divide-y divide-slate-200 dark:divide-slate-800">
                {invoices.map((invoice: any) => (
                  <li key={invoice.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="p-2 bg-slate-100 dark:bg-slate-800 rounded-md">
                        <FileText className="h-5 w-5 text-slate-600 dark:text-slate-400" />
                      </div>
                      <div>
                        <p className="font-medium text-slate-900 dark:text-slate-100">Invoice #{invoice.invoice_number}</p>
                        <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(invoice.date).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-medium text-slate-900 dark:text-slate-100">₹{invoice.total_amount?.toLocaleString('en-IN')}</p>
                      <button className="text-sm text-blue-600 dark:text-blue-400 hover:underline mt-1">Download PDF</button>
                    </div>
                  </li>
                ))}
              </ul>
